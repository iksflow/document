...
```
검색 대상에 여러 개의 패턴을 한 번에 적용시키고 싶을 때는 `-e 패턴`을 여러 개 정의할 수 있습니다.  
...
* -c, --count  
    ...
    `-c` 옵션은 패턴과 일치하는 `라인 수`를 출력하는데, `문자열 개수`가 아니라는 점을 반드시 기억해야 합니다.   
...
* -n, --line-number  
    ...
    27:# Contacts
    ```
    `-n`은 패턴과 일치하는 라인의 `라인 넘버`를 표시합니다.
...
* -v, --invert-match  
    ...
    그 결과 `주석`과 `빈 라인`이 사라진 깔끔한 내용을 얻을 수 있었습니다.  
    ...
    그래서 `^$`은 시작과 끝에 어떠한 문자도 들어있지 않은 `빈 라인`을 의미합니다.  
...
* --color  
    ...
    `--color`는 패턴과 일치하는 부분을 하이라이팅해서 보여줍니다.  
