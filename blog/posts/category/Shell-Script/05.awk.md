...
참고로 awk라는 이름은 이를 개발한 Alfredo **A**ho, Peter **W**einberger, Brian **K**ernighan 의 이름을 따서 만들어졌습니다.


기본 문법과 패턴 그리고 액션에 대해 알아볼 것이다.
패턴 어떤 값을 추출할 것인지
액션 추출한 값을 어떻게 출력할 것인지
...
`grep`이나 `find`를 사용해본 적이 있다면 익숙한 방법일텐데요.  
